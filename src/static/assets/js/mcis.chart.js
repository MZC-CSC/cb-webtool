...
function showOnDemandMonitoring(vmChart, mcis_id, vm_id, metric, interval, agent_ip) {
    $("#canvas_vm").empty();
    vmChart = setVmChartInit(vmChart, "canvas_vm", metric)
    timerId = setInterval(() => getVmOnDemandMetric(vmChart, metric, mcis_id, vm_id, metric, agent_ip), interval * 1000)
}
...
var colorSet = [
    ...
    'rgb(110, 133, 183)',
    'rgb(251, 197, 197)']
function updateChart(vmChart, time, key, datasets) {
    labels.push(time)

    appendData.labels = labels // x축
    ...
    if (data_set.length == 0) {
        ...
        for (var label in datasets) {
            ...
            dt.backgroundColor = color;
            dt.fill = false;
            dt.data = [datasets[label]]
            data_set.push(dt)
            idx++
        }
    } else {
        data_set.map((prevdata, index) => {
            prevdata.data.push(datasets[key[index]])
        })
    }

    appendData.datasets = data_set

    vmChart.data = appendData
    vmChart.update()

}

function getVmOnDemandMetric(vmChart, target, mcisID, vmID, metric, agent_ip) {
    console.log("====== Start GetMetric ====== ")
    console.log("ondemand metric: ", metric);


    // vmChart = setVmChartInit(vmChart, chartTarget, target)


    var url = "/operation/manages/mcismng/proc/vmondemandmonitoring/" + agent_ip
    console.log("Request URL : ", url)
    ...
    axios.post(url, {
        ...
        updateChart(vmChart, data.time, key, values)
